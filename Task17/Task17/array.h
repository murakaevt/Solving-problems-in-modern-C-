...
return array_ptr_[row*col_+col];
}
...
void Permutation(std::size_t row1, std::size_t col1, std::size_t row2, std::size_t col2){
    if(row1 < row_ && col1 < col_ && row2 < row_ && col2 < col_){
        Type buf = array_ptr_[row1*col_+col1];
array_ptr_[row1*col_+col1] = array_ptr_[row2*col_+col2];
array_ptr_[row2*col_+col2] = buf;
    } 
}
...
void Permutation(Type& value1, Type& value2){
    ...
    for(int row = 0; row < row_; ++row){
        for(int col = 0; col < col_; ++col){
            if(array_ptr_[row*col_+col] == value1){
                buf1 = &array_ptr_[row*col_+col];
            } else if(array_ptr_[row*col_+col] == value2){
                buf2 = &array_ptr_[row*col_+col];
            }
        }
    }
...
void Print(){
   ...
   for(int row = 0; row < row_; ++row){
        for(int col = 0; col < col_; ++col){
            std::cout << array_ptr_[row*col_+col] << "  ";
        }
       std::cout << std::endl;
    }
}
    ...
    ConstIterator end() const noexcept {
        return ConstIterator(Get() + row_*col_);
    }
